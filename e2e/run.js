...
async function main() {
  ...
  await scenario('CUT-1', 'The desk cut-off in Settings is what stops bidding', async () => {
    /* The thing the desk could not do: take an offer with the ordinary
     * 09:15-15:15 window and move the line by changing ONE setting.
     *
     * Written against the clock rather than against a fixed hour, so it proves
     * the same thing whenever the suite is run: a cut-off a minute in the past
     * refuses, a cut-off a few minutes ahead accepts, and the offer's own close
     * time - 15:15, whether that is behind us or ahead - never decides it. */
    ...
    const iss = (await POST('/api/issues', {
      symbol: 'CUTOFF' + Math.floor(Math.random() * 100000), company: 'Cut-off Test Ltd',
      isin: 'INE522F01014', exchange: 'BSE', bse_scrip_code: '500011',
      floor_price: 400, cut_price_min: 400, tick: 0.05, lot: 1, cutoff_flag: true,
      hni_open: minusH(6), hni_close: plusH(2),
      ret_open: minusH(6), ret_close: plusH(2)
    }, { session: 'desk' })).json.issue;

    const bid = { issue_id: iss.id, client_ucc: 'ASH1001', exchange: 'BSE',
                  category: 'Retail', qty: 10, price: 400 };
    const errsAt = async (cutoff) => {
      const set = await PUT('/api/settings', { key: 'daily_cutoff', value: cutoff }, { session: 'desk' });
      if (set.status !== 200) throw new Error('could not set the cut-off to ' + cutoff);
      const r = await POST('/api/bids/validate', bid, { session: 'desk' });
      return ((r.json && r.json.errors) || []).join(' ');
    };

    const past = await errsAt(hhmm(-2));
    must(/cut-off/i.test(past), 'a bid was accepted with the cut-off two minutes in the past: ' + past);

    const ahead = await errsAt(hhmm(+30));
    must(!ahead, 'moving the cut-off thirty minutes ahead did not open the bid: ' + ahead);

    await PUT('/api/settings', { key: 'daily_cutoff', value: '23:59' }, { session: 'desk' });
    return { detail: 'refused at ' + hhmm(-2) + ', accepted at ' + hhmm(30) + ' — one setting, no code' };
  }, { expected: 'the setting had no effect at all while the offer close time decided the hour' });

  await scenario('CUT-2', 'The offer still decides which DAYS it runs', async () => {
    const past = (await POST('/api/issues', {
      symbol: 'PASTWIN' + Math.floor(Math.random() * 100000), company: 'Closed Yesterday Ltd',
      isin: 'INE522F01014', exchange: 'BSE', bse_scrip_code: '500012',
      floor_price: 400, cut_price_min: 400, tick: 0.05, lot: 1, cutoff_flag: true,
      hni_open: minusH(50), hni_close: minusH(26),
      ret_open: minusH(50), ret_close: minusH(26)
    }, { session: 'desk' })).json.issue;

    await PUT('/api/settings', { key: 'daily_cutoff', value: '23:59' }, { session: 'desk' });
    const r = await POST('/api/bids/validate', {
      issue_id: past.id, client_ucc: 'ASH1001', exchange: 'BSE',
      category: 'Retail', qty: 10, price: 400
    }, { session: 'desk' });
    const errs = ((r.json && r.json.errors) || []).join(' ');
    must(errs, 'an offer that closed yesterday took a bid because the cut-off was late');
    must(/closed/i.test(errs), 'the refusal does not say the offer is closed: ' + errs);
