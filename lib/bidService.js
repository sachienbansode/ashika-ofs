...
const { validateBid, bidValue, makeRef, marketState, closedMessage, bidExchange } = require('./domain');
...
async function insertBid(b, ctx, placedBy, placedById, branchCode) {
  ...
  try {
    ...
    // 23505 on the REF alone. A duplicate live bid is a different constraint and a
    // different answer — it must keep travelling up to the 409 the caller sends.
    if (e && e.code === '23505' && /ref/.test(String(e.constraint || ''))) return attempt();
    throw e;
  }
}
...
async function updateBid(before, b, ctx, exchOrderNo) {
  ...
  return one(
    ...
    [b.qty, b.price, b.is_cutoff, value, b.category, exch, exchOrderNo || null, before.id]);
}
