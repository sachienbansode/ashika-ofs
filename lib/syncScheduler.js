...
const { capability } = require('./issueSource');
const archiver = require('./archiver');
const circulars = require('./circularWatch');
const exportMail = require('./exportMailer');
...
async function status(now) {
  ...
  return {
    ...
    capability: { NSE: capability('NSE'), BSE: capability('BSE') },
    last_run: last || null,
...
/**
 * Poll the NSE circulars feed. Separate from the exchange pull and on its own timer:
 * a circular can be published at any hour, and this costs one conditional GET, so it
 * runs whether or not the market is open and whether or not auto-pull is on.
 */
async function circularTick() {
  const s = await settings.all();
  if (String(s.circulars_enabled || '1') !== '1') return null;

  const base = Math.max(5, Number(s.circulars_poll_minutes) || 15);
  const st = await circulars.state();

  // After a run of failures the wait doubles each time, to a four-hour ceiling. A
  // poller that keeps knocking every fifteen minutes for a week after being blocked
  // is what gets an address blocked harder.
  const every = circulars.backoffMinutes(base, (st && st.consecutive_failures) || 0);
  if (st && st.last_poll_at && Date.now() - new Date(st.last_poll_at).getTime() < every * 60000) return null;

  const out = await circulars.poll();
  await circulars.recordRun(out, 'schedule', 'scheduler');
  if (out.inserted) {
    console.log('[circulars] ' + out.inserted + ' new OFS circular(s)' +
      (out.issues_made ? ', ' + out.issues_made + ' provisional issue(s) created' : '') +
      (out.alerted ? ', ' + out.alerted + ' alerted' : ''));
  }
  return out;
}
...
async function tick() {
  ...
  try {
    ...
    await circularTick().catch((e) => console.error('[circulars] poll failed:', e.message));
    /* The day's exchange files, emailed at the configured IST time. On this timer
     * rather than its own for the same reason everything else is: one minute tick
     * that reads the database, so the time can be changed from Settings and takes
     * effect on the next minute with no restart and no cron entry on the VM to
     * keep in step. */
    await exportMail.tick().catch((e) => console.error('[export-mail] tick failed:', e.message));
...
module.exports = { start, stop, tick, archiveTick, circularTick, status, everyMinutes, MIN_MINUTES, MAX_MINUTES };
