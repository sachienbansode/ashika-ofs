...
/**
 * Errors the application raises deliberately, which are not database errors and
 * must not be reported as 500s.
 *
 * Both come from the bid write path, and both are answers a person can act on:
 * `margin_exceeded` is the margin re-check inside the per-client lock — the one
 * that catches two simultaneous bids sharing the same free margin — and
 * `stale_bid` is a modify or a withdrawal that lost a race to the other. Shaped
 * like validateBid's own 422 so the screens report them the way they already
 * report a rejected bid.
 */
const APP_ERRORS = { margin_exceeded: 422, stale_bid: 409 };

function send(res, next, e) {
  if (e && APP_ERRORS[e.code]) {
    return res.status(e.status || APP_ERRORS[e.code]).json({
      error: e.code === 'margin_exceeded' ? 'validation_failed' : e.code,
...
module.exports = { translate, send, label, CONSTRAINTS, COLUMNS, APP_ERRORS };
