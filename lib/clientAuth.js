...
/**
 * Find the client(s) matching ONE identifier - a registered mobile OR a registered
 * email. Either side of the join may hold it: dwh.tbl_user_info and
 * stg.ask_clientmast do not always agree, and the client master is the fuller record.
 *
...
async function findClients(identifier) {
  ...
  const mob = normMobile(identifier);
  const eml = normEmail(identifier);

  const where = kind === 'mobile'
    ? `(right(regexp_replace(COALESCE(u.mobile,''),'[^0-9]','','g'),10) = $1
        OR right(regexp_replace(COALESCE(c.mobile,''),'[^0-9]','','g'),10) = $1)`
    : `(lower(btrim(COALESCE(u.email,''))) = $1 OR lower(btrim(COALESCE(c.email_id,''))) = $1)`;
