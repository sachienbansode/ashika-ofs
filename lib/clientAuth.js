...
/**
 * Too many MISSES from one address.
 *
 * throttled() above counts rows in the CHALLENGE table, and a miss creates no
 * challenge - so the one counter that mattered for enumeration never moved for
 * exactly the requests doing the enumerating. That hole is the whole reason the
 * sign-in page had to answer identically whether or not an identifier belonged to
 * a client: naming a miss is only safe while nobody can make misses at scale.
 *
 * The attempt LOG has recorded every miss from the start, with the address on it
 * and an index over (ip, at). Counting those closes the hole, and the numbers are
 * chosen so the two populations do not overlap: somebody mistyping their own
 * client code gets three or four goes at it and never comes near ten, while a
 * script walking a range of codes - and Ashika's are short and numeric - reaches
 * ten inside a second.
 */
const MISS_LIMIT = 10;
const MISS_WINDOW_MIN = 15;

async function missThrottled(ip) {
  if (!ip) return false;
  const r = await one(
    `SELECT count(*)::int AS n
       FROM ${SCHEMA}.ofs_client_login_log
      WHERE ip = $1
        AND reason = 'no_match'
        AND at > now() - ($2 || ' minutes')::interval`,
    [ip, String(MISS_WINDOW_MIN)]);
  return !!r && r.n >= MISS_LIMIT;
}
...
async function createChallenge({ identifier, mobile, email, uccs, ip, userAgent }) {
  ...
  const fixed = testMode(cfg);
  const code = fixed ? testOtp() : generateOtp();
  ...
  await query(
    ...
    [ref, String(identifier || '').trim().toLowerCase(), normEmail(email), uccs,
     hash(code), OTP_MAX_ATTEMPTS, to, fixed ? 'test' : 'both',
     String(OTP_TTL_MIN), ip || null, (userAgent || '').slice(0, 300)]);
  return { ref, code, expiresInMin: OTP_TTL_MIN, sentTo: to, testMode: fixed };
}
...
async function verifyChallenge(ref, code) {
  ...
  const row = await one(
    `UPDATE ${SCHEMA}.ofs_client_otp
      ...
      RETURNING ref, otp_hash, uccs, branch_codes, actor_type, attempts, max_attempts, mobile, email`, [ref]);
  ...
  return {
    ok: true,
    uccs: row.uccs || [],
    // Resolved when the code was issued, never taken from the request. A branch
    // sign-in chooses from this list; it cannot nominate a branch of its own.
    branchCodes: row.branch_codes || [],
    actorType: row.actor_type || 'client',
...
module.exports = {
  ...
  maskEmail, maskMobile, identifierKind, findClients, throttled, missThrottled, resendWait,
  MISS_LIMIT, MISS_WINDOW_MIN,
