 ...
 *
 * The desk cut-off is the authority on the TIME bidding stops, on every day. Which
 * DAYS an offer runs is the issue record's business, and lib/domain.js catStatus
 * applies this same cut-off to the offer's own close day - one setting, read in
 * two places, so the two can never disagree about when the desk has stopped.
 */
