...
/**
 * Where an issue's own window falls on today, in IST minutes past midnight.
 *
 * A window that opened on an earlier day is open from 00:00 today; one that closes
 * on a later day runs to midnight tonight. Either boundary that is not today and
 * not on the permissive side shuts the day completely.
 */
function windowToday(ist, open, close) {
  const at = (v, whenBefore, whenAfter) => {
    const d = v instanceof Date ? v : new Date(v);
    if (isNaN(d)) return null;
    const p = istNow(d);
    if (p.date === ist.date) return p.minutes;
    return p.date < ist.date ? whenBefore : whenAfter;
  };
  return {
    open: open == null ? null : at(open, 0, 24 * 60),          // opened earlier -> from 00:00
    close: close == null ? null : at(close, 0, 24 * 60)        // closes later   -> to 24:00
  };
}
 ...
 * `reason` is null when open, otherwise one of holiday | weekend | before_open |
 * after_cutoff | after_close.
 *
 * `issueWindow`, when given as { open, close }, is the window the desk actually
 * works to: an OFS runs to the close time on its own issue record, and a single
 * desk-wide cut-off applied to every issue is the wrong instrument. An offer whose
 * retail leg runs to 17:15 was being refused from 15:15 because of a setting that
 * knows nothing about that offer. So the issue's own times replace both the
 * session end and the daily cut-off for that bid; the holiday and trading-day
 * gates still apply, because no OFS runs on a day the exchange is shut.
 *
 * Without it - the dashboard clock, the scheduler - the settings decide, as before.
 */
function marketState(settings, now, issueWindow) {
  const s = settings || {};
  const ist = istNow(now);

  let open = toMinutes(s.market_open) != null ? toMinutes(s.market_open) : 9 * 60 + 15;
  ...
  let effective = cutoff != null ? cutoff : close;
  let byIssue = false;

  if (issueWindow && (issueWindow.open != null || issueWindow.close != null)) {
    const w = windowToday(ist, issueWindow.open, issueWindow.close);
    if (w.open != null) { open = w.open; byIssue = true; }
    if (w.close != null) { effective = w.close; byIssue = true; }
  }
  ...
  const base = {
    ...
    cutoffApplies: !byIssue && cutoff != null && cutoff !== close,
    byIssue,
    minutesLeft: 0
  };
  ...
  if (ist.minutes >= effective) {
    return Object.assign(base, {
      open: false,
      reason: byIssue ? 'after_issue_close'
            : cutoff != null && cutoff <= close ? 'after_cutoff'
            : 'after_close'
    });
  }
...
function closedMessage(st) {
  switch (st.reason) {
    ...
    case 'after_issue_close':
                        return 'Bidding closed at ' + st.effectiveClose + ' IST for this offer. No further bids are accepted.';
...
module.exports = { istNow, toMinutes, tradingDays, holidaySet, windowToday, marketState, closedMessage };
