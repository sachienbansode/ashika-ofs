 ...
 */
const { toCsv, toPipe, sha256, stamp, effectivePrice, actionCodeFor } = require('./common');
...
/** N new · M modify · D delete — BSE's codes, not NSE's. */
function actionCode(bid) { return actionCodeFor(EXCHANGE, bid); }
...
function build(bids, settings, opts) {
  ...
  const from = (part - 1) * MAX_ROWS, to = part * MAX_ROWS;
  const body = all.slice(from, to);
  // Which bids these lines are, in file order. The desk's preview shows when each
  // was placed and by whom beside the row; without this it would have to guess the
  // pairing, and a 100-row part boundary would make that guess wrong.
  const bidIds = bids.slice(from, to).map((b) => b.id);
