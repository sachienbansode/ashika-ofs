...
function build(bids, settings, opts) {
  opts = opts || {};
  const s = settings || {};
  // Whether NSE expects a header row is not in the protocol, so it is a setting
  // rather than a decision baked in here. BSE documents that it has none; if NSE
  // turns out to be the same, this is one toggle and no code change.
  const withHeader = opts.noHeader === true ? false
    : opts.noHeader === false ? true
    : String(s.nse_header_row == null ? '1' : s.nse_header_row) === '1';
  const body = (bids || []).map((b) => row(b, b.issue, settings));
  const rows = withHeader ? [HEADER].concat(body) : body;
  ...
  return {
    ...
    unverified: 'Two things are inferred, not documented: the column ORDER (taken from the '
      + 'order v1.3.0 lists the fields in) and whether a header row is expected. '
      + 'Confirm both against circular NSE/CMTR/72975 before the first live upload.'
  };
}
