...
async function get(url, { referer, jar, timeoutMs = 15000 } = {}) {
  ...
  try {
    ...
    // "fetch failed" is Node's message for every network problem there is. The cause
    // chain carries the code that says which one, and they need different fixes —
    // "HTTP 0" on its own told the desk nothing.
    const cause = (e && e.cause) || {};
    const code = cause.code || e.code || (e.name === 'AbortError' ? 'TIMEOUT' : '');
    const why = {
      ENOTFOUND:    'DNS could not resolve the host',
      EAI_AGAIN:    'DNS lookup timed out',
      ECONNREFUSED: 'connection refused',
      ETIMEDOUT:    'connection timed out — outbound HTTPS is probably blocked',
      TIMEOUT:      'no response within the timeout — outbound HTTPS is probably blocked',
      ECONNRESET:   'connection reset mid-request',
      EHOSTUNREACH: 'no route to the host',
      ENETUNREACH:  'the network is unreachable — this server has no outbound internet'
    }[code];
    return {
      ok: false, status: 0, contentType: '', body: '', url,
      code: code || null,
      error: why ? (why + (code ? ' (' + code + ')' : '')) : e.message
    };
  } finally {
    clearTimeout(t);
  }
