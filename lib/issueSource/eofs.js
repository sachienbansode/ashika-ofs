...
function toIssue(sec, settings) {
  ...
  // No basePrice is NOT a rejection. NSE's own FAQ (v3.0, Q12) says the floor price
  // "is not declared to the market", so an OFS with none is normal, not malformed.
  // It is stored as NULL and the desk fills it in if and when it is announced.
  ...
  return {
    issue: {
      ...
      exchange: EXCHANGE,
      floor_price: floor,                 // may be null — see above
      cut_price_min: floor,
