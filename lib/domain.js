...
function validateBid(issue, bid, ctx) {
  ...
  if (issueExch === 'BOTH') {
    if (!bidExch) e.push('Choose the exchange for this bid — ' + issue.symbol + ' is offered on NSE and BSE, and one bid reaches one of them.');
    else if (bidExch !== 'NSE' && bidExch !== 'BSE') e.push('Exchange must be NSE or BSE.');
...
module.exports = { CATS, win, minPrice, floorDisclosed, catStatus, issueStatus, isMultiple, istMinutes,
  bidExchange, openOnDay, issueOpenOnDay,
