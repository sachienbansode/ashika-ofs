 ...
 */
function openOnDay(issue, cat, day) {
...
function issueOpenOnDay(issue, day) {
  return openOnDay(issue, 'Retail', day) || openOnDay(issue, 'HNI', day);
}
...
function validateBid(issue, bid, ctx) {
  ...
  /* A bid may only be placed while this OFFER is open.
   *
   * The window on the issue record is the authority, not a desk-wide cut-off
   * time: an offer whose retail leg runs to 17:15 must take bids until 17:15,
   * and one that shuts at 13:00 must stop at 13:00. A single setting applied to
   * every issue was refusing the first and accepting the second. The holiday and
   * trading-day gates still come from settings — see lib/marketHours.js. */
  const catWin = win(issue, cat);
  const mkt = marketState(s, now, { open: catWin.open, close: catWin.close });
