...
/**
 * When bidding on this offer stops TODAY, and whether it already has.
 *
 * effectiveWin answers "when does this offer finish": its own close day, at the
 * desk's cut-off hour. That is right on the close day and wrong on every day
 * before it. An offer running to Friday is not open continuously until Friday —
 * it takes bids each day until the cut-off and is shut overnight. validateBid has
 * always known this (it asks marketState about TODAY as well as catStatus about
 * the offer), but nothing told the SCREEN, so the panel printed the offer's own
 * close and the refusal printed the cut-off. Two times on one screen, disagreeing,
 * with no way for the desk to tell which one was true.
 *
 * Returns null once the offer itself is over — there is no "today" left for it.
 */
function stopsToday(issue, cat, now, settings) {
  now = now || new Date();
  const w = effectiveWin(issue, cat, settings);
  if (isNaN(w.open) || isNaN(w.close)) return null;
  if (now > w.close) return null;
  const today = atIstTime(now, settings && settings.daily_cutoff, SESSION_CLOSE);
  // The earlier of the two: on the close day they are the same moment, and on any
  // day before it the cut-off is what stops bidding.
  return new Date(Math.min(w.close.getTime(), today.getTime()));
}

/**
 * The window fields every screen needs, computed once here rather than four times
 * in four routes. `_eff` is the offer's close with the cut-off hour applied — the
 * time to PRINT, because it is the time that will be enforced.
 */
function windowFields(issue, now, settings) {
  now = now || new Date();
  const r = effectiveWin(issue, 'Retail', settings);
  const h = effectiveWin(issue, 'HNI', settings);
  return {
    ret_close_eff: isNaN(r.close) ? null : r.close.toISOString(),
    hni_close_eff: isNaN(h.close) ? null : h.close.toISOString(),
    ret_stops_today: iso(stopsToday(issue, 'Retail', now, settings)),
    hni_stops_today: iso(stopsToday(issue, 'HNI', now, settings)),
    daily_cutoff: (settings && settings.daily_cutoff) || SESSION_CLOSE
  };
}
...
module.exports = { CATS, win, effectiveWin, atIstTime, stopsToday, windowFields,
  minPrice, floorDisclosed, catStatus,
